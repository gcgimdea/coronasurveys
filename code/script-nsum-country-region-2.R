...
# to population.
#
# By default, all regions found from data source (1) are processed, regardless 
# of what the `regiontype` indicator suggests. However, if no file is found for 
# that particular regioncode, the region will be ignored.
#
...
find_regions <- function(country) {
    ...
    # NOTE: Output may contain regions for which no data exists;
    #       following scripts must check and account for this

    rtp[countrycode == country][, 2][[1]]
