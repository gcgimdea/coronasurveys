...
def policy(prescribed_ips, costs):
    ...
    if prescribed_ips[stay_at_home_col] >= 1:  # == IP_MAX_VALUES[IP_COLS[stay_at_home_col]]:
        ...
        prescribed_ips[schools_col] = min(max(IP_MAX_VALUES[schools_col_key] // max(1, math.ceil(costs[schools_col])), 1),
                                          prescribed_ips[schools_col])  # 0
        prescribed_ips[workplaces_col] = min(max(IP_MAX_VALUES[workplaces_col_key] // max(1, math.ceil(costs[workplaces_col])),
                                                 1), prescribed_ips[workplaces_col])  # 0
        prescribed_ips[public_events_col] = min(max(IP_MAX_VALUES[public_events_col_key] //
                                                    max(1, math.ceil(costs[public_events_col])), 1),
                                          prescribed_ips[public_events_col])  # 0
        prescribed_ips[gatherings_col] = min(max(IP_MAX_VALUES[gatherings_col_key] // max(1, math.ceil(costs[gatherings_col])), 1),
