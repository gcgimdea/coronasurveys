...
plot_region <- function(country = "IN", region = "Rajasthan")
{
  ...
  p1 <- ggplot(data = df_umd, aes(x = date, color = ""))  +
    ...
    geom_line(aes(y = p_cli_14days*100000, color = "UMD CLI"),
              linetype = "solid", size = 1, alpha = 0.6) +
    geom_ribbon(aes(ymin = p_cli_14days_low*100000,
                    ymax = p_cli_14days_high*100000),
                alpha = 0.1, color = "green", size = 0.1, fill = "green") +
    # geom_point(aes(y = p_cli_local*100000, color = "UMD CLI Indirect"), alpha = 0.5, size = 2) +
    geom_line(aes(y = p_cli_local_14days*100000, color = "UMD CLI Indirect"),
              linetype = "solid", size = 1, alpha = 0.6) +
    geom_ribbon(aes(ymin = p_cli_local_14days_low*100000,
                    ymax = p_cli_local_14days_high*100000),
