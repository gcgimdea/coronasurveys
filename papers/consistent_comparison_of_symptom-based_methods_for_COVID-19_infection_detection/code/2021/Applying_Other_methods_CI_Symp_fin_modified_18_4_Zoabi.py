...
#     }
    ...
    result = result.rename(index={0: "AUC", 1: "F1_score",2: "Specificity",3: "Sensitivity", 4: "Precision"})
    return result
...
def Israel_colum(df_total):
    if df_total['B5.1'] == 1:
        val = 1
    elif df_total['B15_2.1'] == 1:
        val = 1
    else:
        val = 0
    return val
...
def filter_data(iso2): #,number
    ...
    #My jupyter docker
    #df_1 = readRDS("C:/Users/Administrator/Documents/Data_2021_dummies/2021-Q1/dummies/" + iso2 + ".rds")
    df_1 = readRDS("/home/coronasurveys/UMD/2021-Q1/dummies/" + iso2 + ".rds")
    df = pandas2ri.rpy2py_dataframe(df_1)
    #df = df.sample(10000)
    ...
    df['Israel_colum'] = df['B5.1']
    df_1 = df
    #df_2 = readRDS("C:/Users/Administrator/Documents/Data_2021_dummies/2021-Q2/dummies/" + iso2 + ".rds")
    df_2 = readRDS("/home/coronasurveys/UMD/2021-Q2/dummies/" + iso2 + ".rds")
    df = pandas2ri.rpy2py_dataframe(df_2)
    #df = df.sample(10000)
    print("before removing non tested " + str(df.shape))
    df = df[(df['B7.1']==1) & (df['B8.NA']==0) & (df['B8.3']==0)]
    print("after removing non tested " + str(df.shape))
    ...
    #print(df.columns.tolist())
    print(df.columns.tolist())
    df['Israel_colum'] = df.apply(Israel_colum, axis=1)
    df_2 = df
    #df_3 = readRDS("C:/Users/Administrator/Documents/Data_2021_dummies/2021-Q3/dummies/" + iso2 + ".rds")
    df_3 = readRDS("/home/coronasurveys/UMD/2021-Q3/dummies/" + iso2 + ".rds")
    df = pandas2ri.rpy2py_dataframe(df_3)
    #df = df.sample(10000)
    print("before removing non tested " + str(df.shape))
    df = df[(df['B7.1']==1) & (df['B8.NA']==0) & (df['B8.3']==0)]
    print("after removing non tested " + str(df.shape))
    ...
    df['Israel_colum'] = df.apply(Israel_colum, axis=1)
    df_3 = df
    #df_4 = readRDS("C:/Users/Administrator/Documents/Data_2021_dummies/2021-Q4/dummies/" + iso2 + ".rds")
    df_4 = readRDS("/home/coronasurveys/UMD/2021-Q4/dummies/" + iso2 + ".rds")
    df = pandas2ri.rpy2py_dataframe(df_4)
    #df = df.sample(10000)
    print("before removing non tested " + str(df.shape))
    df = df[(df['B7.1']==1) & (df['B8.NA']==0) & (df['B8.3']==0)]
    print("after removing non tested " + str(df.shape))
...
def Israel_preparation(X,Y):
    ...
    Df_try_65 = X[["B1_2.1","B1_1.1","B1_7.1","B1_3.1","B1_12.1","age_65","E3.1","Israel_colum"]]
    Df_try_55 = X[["B1_2.1","B1_1.1","B1_7.1","B1_3.1","B1_12.1","age_55","E3.1","Israel_colum"]]
    y_test = Y
    #Dropping NA
...
def Other_research(train_x,test_x,train_y,test_y):
    ...
    #print("\n############### Symptom-based early-stage differentiation between SARS-CoV-2 versus other respiratory tract infectionsUpper Silesia pilot study\n")
    print("\n Symptom-based early-stage differentiation between SARS-CoV-2 versus other respiratory tract infections \n")
...
def Boostraping_others_methods(df,df_is,n_iterations):
    # configure bootstrap
    # run bootstrap
...
for i in countries:
    ...
    df_DE_other_Metrics = Boostraping_others_methods(Df_try_DE_1,Df_try_is,n_iterations)
    if(i == "AU"):
        pd.options.display.max_columns = None
        print(df_DE_other_Metrics.head())
        #df_DE_other_Metrics.to_csv("df_AU_other_Metrics_CI_Symp_fin.csv")
        print("Fin " + str(i))
    elif(i == "CA"):
        df_DE_other_Metrics.to_csv("df_CA_Zoabi_Metrics_CI_Symp.csv")
        print("Fin " + str(i))
    elif(i == "DE"):
        df_DE_other_Metrics.to_csv("df_DE_Zoabi_Metrics_CI_Symp.csv")
        print("Fin " + str(i))
    elif(i == "JP"):
        df_DE_other_Metrics.to_csv("df_JP_Zoabi_Metrics_CI_Symp.csv")
        print("Fin " + str(i))
    elif(i == "ZA"):
        df_DE_other_Metrics.to_csv("df_ZA_Zoabi_Metrics_CI_Symp.csv")
    elif(i == "BR"):
        df_DE_other_Metrics.to_csv("df_BR_Zoabi_Metrics_CI_Symp.csv")
